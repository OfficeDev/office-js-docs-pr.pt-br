...
Como **sync()** é uma operação assíncrona que retorna uma promessa, você sempre deve **retornar** a promessa (no JavaScript). Isso garante que a operação **sync()** seja concluída antes que o script continue sendo executado. Para saber mais sobre como otimizar o desempenho com **sync()**, confira [Otimização do desempenho da API JavaScript do Excel](https://docs.microsoft.com/office/dev/add-ins/excel/performance).
...
* [Otimização de desempenho do da API JavaScript do Excel](https://docs.microsoft.com/office/dev/add-ins/excel/performance)
