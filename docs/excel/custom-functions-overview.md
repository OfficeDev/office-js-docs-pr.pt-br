...
1.  Instale o Office 2016 para Windows e ingresse no programa [Office Insider](https://products.office.com/pt-BR/office-insider).
...
Após o registro, funções personalizadas serão disponibilizadas em todas as pastas de trabalho (não apenas aquela onde o suplemento foi iniciado inicialmente) para um usuário. As funções são exibidas no menu de preenchimento automático quando o usuário começa a digitá-la.
...
-   Um elemento &lt;`Script`&gt;, o que é obrigatório, mas não usado na versão Developer Preview.
...
1.   Retornar a promessa JavaScript para o Excel.
...
Para funções de streaming, o parâmetro final, `setResult`, não é especificado no código de registro e nunca é exibido no menu de preenchimento automático para usuários do Excel ao inserir a função. É uma função de retorno de chamada usada para passar dados da função para o Excel para atualizar o valor de uma célula. Para que o Excel passe a função `setResult`, é necessário declarar o suporte para streaming durante o registro da função ao configurar o parâmetro `stream` para `true`.
...
-   **Saving data.** `refreshTemperature` é uma função de streaming que lê a temperatura de um determinado termômetro a cada segundo. Novas temperaturas são salvas na variável savedTemperatures.
...
-   Cancelamento, que notificará você quando uma função de streaming não for mais necessária (por exemplo, quando os usuários limparem uma célula). Hoje, as funções não podem determinar quando parar de escrever novos valores na célula.

-   As descrições de URLs de Ajuda e parâmetros ainda não são usadas pelo Excel.

-   A publicação de suplementos na Office Store ou uma implantação centralizada do Office 365 usando funções personalizadas.
