...
* Especificar as permissões que seu suplemento precisa para o AAD v. Ponto de extremidade 2.0 (e, opcionalmente, para o Microsoft Graph). A permissão "perfil" é sempre necessária.
...
* **WebApplicationInfo** – O responsável dos seguintes elementos.
...
* **Escopos** – O responsável de um ou mais elementos de **Escopo**.
...
### <a name="add-client-side-code"></a>Adicionar código do cliente
...
* Obter dados do Microsoft Graph. O código do servidor precisa fazer o seguinte:
...
- `oid` – Um GUID que representa a ID do usuário no Active Directory do Azure.
- `tid` – Um GUID que representa a ID da organização do usuário no Active Directory do Azure.
...
A seguir, um conteúdo decodificado típico de um token de acesso. Para mais informações sobre as propriedades, veja [Referência de tokens do Active Directory do Azure v2.0](https://docs.microsoft.com/en-us/azure/active-directory/develop/active-directory-v2-tokens).
