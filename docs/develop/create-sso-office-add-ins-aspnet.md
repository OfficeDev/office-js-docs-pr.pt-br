...
1. Um **URI de ID do aplicativo** foi gerado na forma "api://{App ID GUID}". Insira a cadeia de caracteres "localhost:44355/" entre as duas barras e o GUID. A ID completa deve ser `api://localhost:44355/{App ID GUID}`. 

    > [!NOTE]
    > A parte do domínio do nome do **Escopo** logo abaixo do **URI da ID do aplicativo** será automaticamente alterada para que haja correspondência. Ela deve ser assim: `api://localhost:44355/{App ID GUID}/access_as_user`.
...
1. Na caixa de diálogo, marque as caixas das seguintes permissões: Somente a primeira é realmente exigida pelo suplemento propriamente dito, mas a biblioteca MSAL usada pelo código de servidor exige `offline_access` e `openid`. A permissão `profile` é necessária para que o host do Office obtenha um token no aplicativo Web do seu suplemento.
    * Files.Read.All
    * offline_access
    * openid
    * perfil
...
2. Na barra de endereços do navegador, você verá um parâmetro de consulta de locatário "tenant" com um valor GUID. Esta é a ID da sua locação do Office 365. Copie e salve esse valor. Você o usará em uma etapa posterior.
