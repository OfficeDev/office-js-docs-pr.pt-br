...
1. Substitua `TODO13A` pelo código a seguir. (Isso cria a primeira parte de um bloqueio condicional *interno*.) Observação sobre o código:
...
1. Substitua `TODO13C` pelo código a seguir. Observação sobre este código:
...
1. Substitua `TODO14` pelo código a seguir. (Isso faz parte do bloqueio condicional *externo* e deve ser colocado imediatamente após o colchete de fechamento da estrutura que começa com `else if (exceptionMessage) {` e com o mesmo nível de recuo.) Observação sobre o código:
...
1. Substitua `TODO1` pelo seguinte código para validar que os escopos especificados no token incluam `access_as_user`.
...
1. Substitua `TODO3` pelo código a seguir. Observação sobre este código:
...
1. Substitua `TODO3a` pelo código a seguir. Observação sobre este código:
...
1. Substitua `TODO3b` e `TODO3c` pelo código a seguir. Observação sobre este código:
...
1. Substitua `TODO3d` pelo código a seguir. Observe que o código exibe a exceção em vez de transmiti-la em uma resposta HTTP personalizada com **HttpStatusCode.Forbidden** (401). O efeito disso é que o ASP.NET enviará sua própria resposta HTTP com o status "500 Erro de Servidor".
...
1. Substitua `TODO5` pelo seguinte. Observação sobre este código:
...
1. Abaixo do método Get, adicione o método a seguir. Sobre este código, observe:  
