...
A caixa de diálogo sempre abre no centro da tela. O usuário pode movê-la e redimensioná-la. A janela é *não modal*: o usuário pode continuar a interagir com o documento no aplicativo do Office do host e com a página host no painel de tarefas, caso haja uma.
...
> [!NOTE]
> - A URL usa o protocolo HTTP**S**. Isso é obrigatório em todas as páginas carregadas em uma caixa de diálogo, não apenas na primeira página carregada.
> - O domínio do recurso da caixa de diálogo é o mesmo que o domínio da página host, que pode ser a página em um painel de tarefas ou o [arquivo de função](https://docs.microsoft.com/office/dev/add-ins/reference/manifest/functionfile?view=office-js) de um comando de suplemento. Isto é necessário: a página, o método do controlador ou outro recurso que seja passado para o método `displayDialogAsync` deve estar no mesmo domínio que a página host.
...
### <a name="take-advantage-of-a-performance-option-in-office-online"></a>Obter benefícios com uma opção de desempenho no Office Online

A propriedade `displayInIframe` é do tipo adicional no objeto de configuração que se pode passar para `displayDialogAsync`. Quando essa propriedade estiver definida como `true` e o suplemento estiver sendo executado em um documento aberto no Office Online, a caixa de diálogo será aberta como um iframe flutuante em vez de uma janela independente, o que torna sua abertura mais rápida. Eis um exemplo:

```js
Office.context.ui.displayDialogAsync('https://myDomain/myDialog.html', {height: 30, width: 20, displayInIframe: true});
```

O valor padrão é `false`, que é o que mesmo omitir a propriedade inteiramente. Se o suplemento não estiver funcionando no Office Online, o `displayInIframe` será ignorado.

> [!NOTE]
> Você **não** deverá usar `displayInIframe: true` se a caixa de diálogo for redirecionada, a qualquer momento, para uma página que não possa ser aberta em um iframe. Por exemplo, as páginas de entrada de muitos serviços Web populares, como Google e Conta da Microsoft, não podem ser abertas em um iframe.

### <a name="send-information-from-the-dialog-box-to-the-host-page"></a>Envie informações da caixa de diálogo para a página host

A caixa de diálogo não consegue se comunicar com a página host no painel de tarefas, a menos que:
...
O código na página de diálogo usa a função `messageParent` para enviar uma mensagem de cadeia de caracteres ou um valor booleano para a página host. A cadeia de caracteres pode ser uma palavra, uma frase, um blob XML, um JSON em formato de cadeia de caracteres ou qualquer outra coisa que possa ser serializada em uma cadeia de caracteres. Eis um exemplo:
...
> [!NOTE]
> - A função `messageParent` é uma das *únicas* duas APIs do Office que podem ser chamadas na caixa de diálogo; a outra é `Office.context.requirements.isSetSupported`. Para saber mais, confira [Especificar hosts do Office e requisitos da API](specify-office-hosts-and-api-requirements.md).
...
A página host deve ser configurada para receber a mensagem. Você pode fazer isso adicionando um parâmetro de retorno de chamada à chamada original de `displayDialogAsync`. O retorno de chamada atribui um manipulador ao evento `DialogMessageReceived`. Eis um exemplo:
...
> [!NOTE]
> - O Office transmite um objeto [AsyncResult]() para o retorno de chamada. Ele representa o resultado de tentativas de abrir a caixa de diálogo, mas não representa o resultado de eventos na caixa de diálogo. Para obter mais informações sobre essa distinção, confira a seção [Manipular erros e eventos](#handle-errors-and-events).
...
Eis um exemplo simples de um manipulador para o evento `DialogMessageReceived`:
...
Se o suplemento precisa abrir uma página diferente do painel de tarefas depois de receber a mensagem, é possível usar o método `window.location.replace` (ou `window.location.href`) como a última linha do manipulador. Eis um exemplo:
...
Para ver exemplos que usam sistemas de mensagens condicionais, confira:
...
Você pode implementar um botão na caixa de diálogo para fechá-la. Para fazer isso, o manipulador de eventos clicável do botão deve usar `messageParent` para informar a página host em que o botão foi clicado. Eis um exemplo:
...
|:-----|:-----|
|12004|O domínio que a URL transmitiu para `displayDialogAsync` não é confiável. O domínio deve ser igual ao da página host (incluindo o protocolo e o número da porta).|
|12005|A URL passada para `displayDialogAsync` usa o protocolo HTTP, o qual é necessário. (Em algumas versões do Office, a mensagem de erro retornada com 12005 é a mesma retornada para 12004.)|
...
|Número do código|Significado|
|:-----|:-----|
|12002|Uma das seguintes opções:<br> – Não existe uma página na URL transmitida para `displayDialogAsync`.<br> A página transmitida para `displayDialogAsync` foi carregada, mas a caixa de diálogo foi direcionada para uma página que ela não consegue localizar nem carregar ou foi direcionada para uma URL com sintaxe inválida.|
|12003|A caixa de diálogo foi direcionada para uma URL com o protocolo HTTP. HTTPS é necessário.|
|12006|A caixa de diálogo foi fechada, geralmente isso ocorre por o usuário ter escolhido o botão **X**.|

Seu código pode atribuir um manipulador para o evento `DialogEventReceived` na chamada para `displayDialogAsync`. Eis um exemplo:
...
## <a name="pass-information-to-the-dialog-box"></a>Transmitir informações para a caixa de diálogo
...
Para ver suplementos de amostra que usam o armazenamento local dessa forma, confira:
...
## <a name="use-the-dialog-apis-to-show-a-video"></a>Usar APIs de Caixa de Diálogo para exibir um vídeo
...
2.  A página video.dialogbox.html deve estar no mesmo domínio que a página host.
...
Para ver um exemplo que usa um vídeo na caixa de diálogo, confira o [padrão de design do vídeo placemat](https://github.com/OfficeDev/Office-Add-in-UX-Design-Patterns-Code/tree/master/templates/first-run/video-placemat) no repositório [padrões de design da experiência do usuário para Suplementos do Office](https://github.com/OfficeDev/Office-Add-in-UX-Design-Patterns-Code).
...
4. O evento `DialogMessageReceived` é acionado na página host, seu manipulador fecha a janela de diálogo e, opcionalmente, faz outro processamento da mensagem.

Para ver suplementos de amostra que usam esse padrão, confira:
...
- [Autenticação de Cliente do Office 365 de Suplementos do Office para AngularJS](https://github.com/OfficeDev/Word-Add-in-AngularJS-Client-OAuth):  recurso que é inicialmente aberto na janela de diálogo é uma página.
...
- Se o usuário ainda não tiver concedido ao aplicativo as permissões necessárias, ele será solicitado a fazê-lo na caixa de diálogo depois de entrar.
