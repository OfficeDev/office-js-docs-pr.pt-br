...
####  <a name="bcc-recipientsjavascriptapioutlook12officerecipients"></a>cco:[Recipients](/javascript/api/outlook_1_2/office.recipients)
...
####  <a name="body-bodyjavascriptapioutlook12officebody"></a>corpo:[Body](/javascript/api/outlook_1_2/office.body)
...
As propriedades [`from`](#from-emailaddressdetailsjavascriptapioutlook12officeemailaddressdetails) e `sender` representam a mesma pessoa, a menos que a mensagem seja enviada por um delegado. Nesse caso, a propriedade `from` representa o delegador, e a propriedade sender, o delegado.
...
O exemplo a seguir define a hora de início de um compromisso no modo de redação usando o método [`setAsync`](/javascript/api/outlook_1_2/office.time#setasync-datetime--options--callback-) do objeto `Time`.
...
| `formData.attachments.url` | Sequência de caracteres | | Usado somente se `type` estiver definido como `file`. O URI da localização para o arquivo. |
...
|Nome| Tipo| Descrição|
|---|---|---|
|`entityType`| [Office.MailboxEnums.EntityType](/javascript/api/outlook_1_2/office.mailboxenums.entitytype)|Um dos valores da enumeração EntityType.|
