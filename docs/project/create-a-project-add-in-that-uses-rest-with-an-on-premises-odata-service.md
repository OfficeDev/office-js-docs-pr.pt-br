...
Veja a seguir os pré-requisitos para a criação de um complemento do painel de tarefas do Project que lê o serviço **ProjectData** de uma instância do Project Web App em uma instalação local do Project Server 2013:
...
- O Project Professional 2013 é necessário para a conexão com o Project Web App. O computador de desenvolvimento deve ter o Project Professional 2013 instalado para habilitar a depuração **F5** com Visual Studio.

    > [!NOTE]
    > O Project Standard 2013 também pode hospedar os complementos do painel de tarefas, mas não pode entrar no Project Web App.
...
1. Execute Visual Studio 2015 como administrador e selecione **Novo Projeto** na página Iniciar.

2. Na caixa **de diálogo** Novo Projeto, expanda os nós **Templates,** **Visual C#** e **Office/SharePoint** e selecione ** Office Add-ins**. Selecione **.NET Framework 4.5.2** na listada na parte superior do painel central e selecione o Office **Add-in** (consulte a próxima captura de tela).
...
6. Na caixa **de diálogo Escolher os aplicativos** host, desempure todas as caixas de seleção, exceto a caixa de seleção **Projeto** (consulte a próxima captura de tela) e escolha **Concluir**.

    *Figura 3. Como escolher o aplicativo host*

    ![Escolher o Project como o único aplicativo host](../images/create-office-add-in.png)

    Visual Studio cria o **projeto HelloProjectOdata** e o **projeto HelloProjectODataWeb.**
...
4. Adicione um ícone para mostrar na lista de menus de menus de **complementos** do Office na guia **PROJECT** da faixa de opções. Você pode adicionar um arquivo de ícone na solução do Visual Studio ou usar uma URL para um ícone. 
...
2. Para ser exibido na lista de **menus drop-ins** do Office, o ícone deve ter 32 x 32 pixels. Por exemplo, instale o SDK do Project 2013, escolha a pasta **Imagens** e adicione o seguinte arquivo do SDK: `\Samples\Apps\HelloProjectOData\HelloProjectODataWeb\Images\NewIcon.png`
...
- Uma tabela exibe os valores médios de custo do projeto, custo real, trabalho e porcentagem concluída. A tabela também compara os valores atuais do projeto ativo com a média. Se o valor atual for maior que a média de todos os projetos, será exibido em vermelho. Se o valor atual for menor que a média, será exibido em verde. Se o valor atual não estiver disponível, a tabela exibirá um **NA azul**.
    ...
    > [!NOTE]
    > Neste exemplo, os dados de custo e trabalho do projeto ativo são derivados dos valores publicados. Se você alterar valores no Project, o **serviço ProjectData** não terá as alterações até que o projeto seja publicado.
...
O modelo para um suplemento de painel de tarefas do Project inclui código de inicialização padrão que foi projetado para demonstrar ações get e set básicas para dados em um documento no caso de um suplemento típico do Office 2013. Como o Project 2013 não dá suporte a ações que escrevem no projeto ativo, e o complemento **HelloProjectOData** não usa o método, você pode excluir o script dentro da função e excluir a função e a função no arquivo HelloProjectOData.js `getSelectedDataAsync` `Office.initialize` `setData` `getData` padrão.

O JavaScript inclui constantes globais para a consulta REST e variáveis globais que são usadas em várias funções. O botão Obter Ponto de **Extremidade do ProjectData** chama a função, que inicializa as variáveis globais e determina se o `setOdataUrl` Project está conectado ao Project Web App.
...
2. Adicionar `setOdataUrl` e funções relacionadas. A `setOdataUrl` função chama e inicializa as `getProjectGuid` `getDocumentUrl` variáveis globais. No método [getProjectFieldAsync](/javascript/api/office/office.document), a função anônima para o parâmetro _de_ retorno de chamada habilita o botão **Comparar** Todos os Projetos usando o método na biblioteca jQuery e exibe a URL do serviço `removeAttr` **ProjectData.** Se o Project não estiver conectado ao Project Web App, a função gera um erro e exibe uma mensagem de erro pop-up. O SurfaceErrors.js inclui o `throwError` método.
   ...
   > [!NOTE]
   > Se você executar Visual Studio no computador do Project Server, para usar a depuração **F5,** descompacte o código após **a** linha que inicializa a variável _pwa global. Para habilitar o uso do método jQuery ao depurar no computador do Project Server, você deve definir o valor `ajax` para a URL do `localhost` PWA. Se você executar Visual Studio em um computador remoto, a  `localhost` URL não será necessária. Before you deploy the add-in, comment out that code.
...
2. Na caixa de diálogo Contas do aplicativo Web do **Project,** **a** lista Contas disponíveis pode ter várias contas do Project Web App, além da conta **local do** computador. Na seção **Ao iniciar**, selecione **Escolher uma conta**.
...
- Execute o add-in novamente, onde você escolhe o perfil do computador local na caixa de diálogo **Logon** quando o Project é iniciado. Abra um arquivo .mpp local e teste o suplemento. Verifique se o suplemento exibe uma mensagem de erro ao tentar acessar o ponto de extremidade **ProjectData**.
...
3. Na guia **PROJECT** da faixa de opções, na listada de **complementos** do Office, selecione **Hello ProjectData** (consulte a Figura 5). O botão **Comparar Todos os Projetos** deve estar desativado.
