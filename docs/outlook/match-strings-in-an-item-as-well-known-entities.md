...
|**EmailAddress**|Endereços de email SMTP.|`String`Objeto JavaScript|
...
|**Url**|Um endereço Web que especifica explicitamente o local de rede e o identificador de um recurso da Web. O Exchange Server não requer o protocolo de acesso no endereço Web e não reconhece URLs que são inseridas no texto do link como instâncias da `Url` entidade. O Exchange Server pode corresponder aos seguintes exemplos: `www.youtube.com/user/officevideos``https://www.youtube.com/user/officevideos` |`String`Objeto JavaScript|
...
Após a chamada `getEntities` , você pode usar a propriedade correspondente do `Entities` objeto para obter uma matriz de instâncias de um tipo de entidade. Dependendo do tipo de entidade, as instâncias na matriz podem ser apenas cadeias de caracteres ou podem mapear para objetos específicos. 

Como o exemplo mostrado na figura anterior, acesse a matriz retornada por `getEntities().addresses[]` para obter endereços no item. A `Entities.addresses` propriedade retorna uma matriz de cadeias de caracteres que o Outlook reconhece como endereços postais. Da mesma forma, a `Entities.contacts` propriedade retorna uma matriz de `Contact` objetos que o Outlook reconhece como informações de contato. A Tabela 1 lista o tipo de objeto de uma instância de cada entidade compatível.
...
Opcionalmente, você pode aplicar uma expressão regular em uma `ItemHasKnownEntity` regra, para que as instâncias de filtro sejam mais futuras de uma entidade e fazer com que o Outlook ative um suplemento somente em um subconjunto das instâncias da entidade. Por exemplo, você pode especificar um filtro para a entidade de rua do endereço em uma mensagem que contenha um CEP do Rio de Janeiro que comece com "021". Para aplicar um filtro nas instâncias de entidade, use os `RegExFilter` `FilterName` atributos e no `Rule` elemento do tipo [ItemHasKnownEntity](../reference/manifest/rule.md#itemhasknownentity-rule) .

De forma semelhante às outras regras de ativação, você pode especificar várias regras a fim de formar uma coleção de regras para seu suplemento. O exemplo a seguir aplica uma operação "AND" em 2 regras: uma `ItemIs` regra e uma `ItemHasKnownEntity` regra. Essa coleção de regras ativa o suplemento sempre que o item atual for uma mensagem e o Outlook reconhecer um endereço no assunto ou no corpo do item.
...
O exemplo a seguir usa `getEntitiesByType` o item atual para definir uma variável `addresses` com os resultados da coleção de regras anterior.
...
O exemplo de regra a seguir `ItemHasKnownEntity` ativa o suplemento sempre que houver uma URL no assunto ou no corpo do item atual, e a URL contiver a cadeia de caracteres "YouTube", independentemente do caso da cadeia de caracteres.
...
O exemplo a seguir usa `getFilteredEntitiesByName(name)` o item atual para definir uma variável `videos` para obter uma matriz de resultados que correspondam à expressão regular na `ItemHasKnownEntity` regra anterior.
...
- Quando o suplemento estiver sendo executado em um cliente avançado do Outlook, espere que o Outlook aplique a `ItemHasKnownEntity` regra ao primeiro megabyte do corpo do item e não ao restante do corpo acima desse limite.
