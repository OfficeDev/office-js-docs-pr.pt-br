...
ms.locfileid: "57093315"
---
# <a name="tips-for-handling-date-values-in-outlook-add-ins"></a>Dicas para lidar com valores de data em suplementos do Outlook

A Office JavaScript usa o objeto Data [JavaScript](https://www.w3schools.com/jsref/jsref_obj_date.asp) para a maioria do armazenamento e recuperação de datas e horas. 
...
O conceito de "hora local" é basicamente determinado pelo navegador e pelo sistema operacional no computador cliente. Por exemplo, na maioria dos navegadores em execução em um computador cliente baseado em Windows, uma chamada javaScript para , retorna uma data com base no fuso horário definido no Windows no computador `getDate` cliente.
...
Conforme descrito nas seções anteriores, como o "horário local" para um usuário no Outlook na Web ou dispositivos móveis pode ser diferente em um cliente rico do Outlook, mas o objeto Data **javaScript** dá suporte à conversão apenas para o fuso horário do computador cliente ou UTC, a API javaScript do Office fornece dois métodos auxiliares: [Office.context.mailbox.convertToLocalClientTime](../reference/objectmodel/preview-requirement-set/office.context.mailbox.md#methods) [e Office.context.mailbox.convertToUtcClientTime](../reference/objectmodel/preview-requirement-set/office.context.mailbox.md#methods).
...
Observe que cuida da diferença entre um cliente Outlook cliente rico e Outlook na Web `convertToLocalClientTime` ou dispositivos móveis:
...
- Se detectar que o aplicativo atual é Outlook na Web ou dispositivos móveis, o método converterá a representação utc correta em um formato de dicionário no fuso horário do EAC, consistente com o restante da interface do usuário Outlook na Web ou `convertToLocalClientTime` dispositivos `Date` móveis.
...
Observe que cuida da diferença entre um cliente Outlook cliente rico e Outlook na Web `convertToUtcClientTime` ou dispositivos móveis:


- Se detectar que o aplicativo atual é um Outlook cliente rico, o método simplesmente converterá a representação do dicionário `convertToUtcClientTime` em um `Date` objeto. Este `Date` objeto é UTC correto, conforme esperado por `displayNewAppointmentForm` .
    
- Se detectar que o aplicativo atual Outlook na Web ou dispositivos móveis, o método converterá o formato de dicionário dos valores de data e hora expressos no fuso horário do EAC em `convertToUtcClientTime` um `Date` objeto. Este `Date` objeto é UTC correto, conforme esperado por `displayNewAppointmentForm` .
