...
O código a seguir recupera um documento do PowerPoint ou do Word como um objeto **File** usando o método **document.getFileAsync()**. Em seguida, ele empacota o objeto **File** resultante, um contador zerado e [sliceCount](http://dev.office.com/reference/add-ins/shared/file.slicecount) em um objeto anônimo. Esse objeto é passado depois para uma função `getSlice` definida localmente. 
