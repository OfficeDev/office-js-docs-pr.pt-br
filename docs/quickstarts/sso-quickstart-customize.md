---
title: Adicionar a funcionalidade Graph Microsoft ao seu projeto de início rápido do SSO
description: Saiba como adicionar a nova funcionalidade Graph Microsoft ao complemento habilitado para SSO que você criou.
ms.date: 01/25/2022
...
ms.locfileid: "63745457"
---
# <a name="add-microsoft-graph-functionality-to-your-sso-quick-start-project"></a>Adicionar a funcionalidade Graph Microsoft ao seu projeto de início rápido do SSO
...
10. No painel que será aberto, escolha **Microsoft Graph** e escolha **Permissões delegadas**.
...
- Atualize o código que analisa a resposta do Microsoft Graph e grava-o no documento ou na mensagem.
...
Conclua as etapas a seguir para o seu add-in, para alterar a URL do Microsoft Graph, os parâmetros e o escopo de acesso e atualize a interface do usuário do painel de tarefas. Essas etapas são as mesmas, independentemente de qual aplicativo Office seus destinos de complemento.
...
2. Em **./manifest.xml**, encontre a linha `<Scope>User.Read</Scope>` próxima ao final do arquivo e substitua-a pela linha `<Scope>Files.Read.All</Scope>`.

3. Em **./src/helpers/fallbackauthdialog.js** (ou em **./src/helpers/fallbackauthdialog.ts** para um projeto TypeScript), `https://graph.microsoft.com/User.Read` `https://graph.microsoft.com/Files.Read.All`localizar a cadeia de caracteres e substituí-la pela cadeia de caracteres , `requestObj` de forma que seja definida da seguinte maneira:
...
4. Em **./src/taskpane/taskpane.html**, `<section class="ms-firstrun-instructionstep__header">` encontre o elemento e atualize o texto dentro desse elemento para descrever a nova funcionalidade do complemento.
...
5. Em **./src/taskpane/taskpane.html**, encontre e substitua ambas as ocorrências da cadeia de `Get My User Profile Information` caracteres pela cadeia de caracteres `Read my OneDrive for Business`.
...
6. Em **./src/taskpane/taskpane.html**, encontre e substitua a cadeia de caracteres `Your user profile information will be displayed in the document.` pela cadeia de caracteres `The names of the top 10 files and folders in your OneDrive for Business will be displayed in the document or message.`.
...
7. Atualize o código que analisa a resposta do Microsoft Graph e grava-a no documento ou na mensagem, seguindo as diretrizes na seção que corresponde ao seu tipo de complemento:
    ...
    - [Alterações necessárias para um Excel (TypeScript)](#changes-required-for-an-excel-add-in-typescript)
    - [Alterações necessárias para um Outlook (JavaScript)](#changes-required-for-an-outlook-add-in-javascript)
...
### <a name="changes-required-for-an-excel-add-in-typescript"></a>Alterações necessárias para um Excel (TypeScript)

Se o seu add-in for um Excel que foi criado com TypeScript, abra **./src/taskpane/taskpane.ts**, `writeDataToOfficeDocument` encontre a função e substitua-a pela função a seguir.
...
### <a name="changes-required-for-an-outlook-add-in-javascript"></a>Alterações necessárias para um Outlook (JavaScript)
...
Se o seu add-in for um Outlook que foi criado com TypeScript, abra **./src/taskpane/taskpane.ts**, `writeDataToOfficeDocument` encontre a função e substitua-a pela função a seguir.
...
Se o seu add-in for um PowerPoint que foi criado com TypeScript, abra **./src/taskpane/taskpane.ts**, `writeDataToOfficeDocument` encontre a função e substitua-a pela função a seguir.
...
Se o seu complemento for um complemento do Word criado com TypeScript, abra **./src/taskpane/taskpane.ts**, `writeDataToOfficeDocument` encontre a função e substitua-a pela função a seguir.
...
Se o seu add-in for um Excel, Word ou PowerPoint de PowerPoint, conclua as etapas na seção a seguir para experimentar. Se o seu Outlook é um Outlook, conclua as etapas na [seção](#outlook) Outlook em vez disso.
...
1. Na pasta raiz do projeto, execute o seguinte comando para criar o projeto, inicie o servidor Web local e o sideload do seu add-in no aplicativo cliente Office selecionado anteriormente.
...
2. No aplicativo cliente do Office que é aberto quando você executar o comando anterior (ou seja, Excel, Word ou PowerPoint), certifique-se de estar conectado com um usuário membro da mesma organização do Microsoft 365 que a conta de administrador do Microsoft 365 que você usou para se conectar ao Azure durante a configuração [do SSO](sso-quickstart.md#configure-sso)  para o aplicativo. Isso estabelecerá as condições apropriadas para que o SSO seja bem-sucedido. 
...
3. No aplicativo cliente do Office, escolha a guia **Página Inicial** e o botão **Mostrar Painel de Tarefas** na faixa de opções para abrir o painel de tarefas do suplemento. A imagem a seguir mostra esse botão no Excel.
    ...
    ![Captura de tela mostrando o botão de complemento realçado Excel faixa de opções.](../images/excel-quickstart-addin-3b.png)
...
6. O add-in lê dados do usuário OneDrive for Business e grava os nomes dos 10 principais arquivos e pastas no documento. A imagem a seguir mostra um exemplo de nomes de arquivo e pasta gravados em uma Excel de trabalho.

    ![Captura de tela mostrando OneDrive for Business informações Excel planilha.](../images/sso-onedrive-info-excel.png)
...
2. Certifique-se de estar conectado ao Outlook com um usuário membro da mesma organização do Microsoft 365 que a conta de administrador do Microsoft 365 que você usou para se conectar ao Azure durante a configuração do [SSO](sso-quickstart.md#configure-sso) para o aplicativo. Isso estabelecerá as condições apropriadas para que o SSO seja bem-sucedido.
